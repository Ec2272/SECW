    ...
    for (let i = 0; i < 5; i++) {
      ...
      const description =
        Math.random() < 0.33
          ...
          : Math.random() < 0.66
            ? "Lab Issue Ticket(s)"
            : "Service Issue Ticket(s)"; // Random ticket type
  ...
  return (
    <div>
      ...
      {Object.keys(groupedRecords).map((description) => (
        <div key={description}>
          <h2
            onClick={() => toggleFolder(description)}
            style={{ cursor: "pointer" }}
            className="TrackTicketTitle"
          >
              ...
              <tbody>
                {groupedRecords[description].map((record) => (
                  <tr key={record.id}>
                    ...
                    <td data-label="Module Code">{record.moduleCode}</td>
                    <td data-label="Date/Time">{record.dateTime}</td>
                    <td data-label="Status">{record.status}</td>
                    <td data-label="Actions">
                      <button
                        className="Uploading"
                        onClick={() => handleActionClick(record.id, "upload")}
                      >
                        Upload
                      </button>
                      <button
                    ...
                    <td data-label="Uploaded Document(s)">
                      {record.uploadedFile && (
                        <div>
